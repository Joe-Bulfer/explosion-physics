...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        */
        ...
        Rectangle cannon = { player.position.x,
                             player.position.y,
                            cannonLength, 20 };
        DrawRectanglePro(cannon, (Vector2){ 0, 10 }, angleDeg, GRAY);
        ...
        DrawText(TextFormat("angleDeg: %f", angleDeg), 80, 330, 30, RED);
        DrawText(TextFormat("angleRad: %f", angleRad), 80, 360, 30, RED);
        DrawText(TextFormat("position: %f, %f", player.position.x,player.position.y), 80, 390, 30, RED);
...
//from https://github.com/raysan5/raylib/blob/master/examples/textures/textures_sprite_explosion.c
